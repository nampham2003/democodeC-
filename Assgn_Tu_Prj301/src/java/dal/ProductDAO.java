...
public class ProductDAO extends DBContext {
    ...
    public List<Product> getAllProducts() {
        ...
        try {
            String sql = "select * from Product_HE161770";
    ...
    public List<Product> getProductsByCategoryId(int categoryId) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select * from Product_HE161770 where Product_HE161770.cateID = ?";
    ...
    public List<Product> getProductsWithPagging(int page, int PAGE_SIZE) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from Product_HE161770 order by id\n"
    ...
    public int getTotalProducts() {
        try {
            String sql = "select count(id)  from Product_HE161770 ";
    ...
    public List<Product> search(String keyword) {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "select *  from Product_HE161770 where name like ?";
    ...
    public Product getProductById(int productId) {
        try {
            String sql = "select *  from Product_HE161770 where id = ?";
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public void inSertProduct(String name, String img, String price, String title, String description, String cid, int sell_ID) {
        try {
            String sql = "INSERT INTO [Product_HE161770]\n"
                    + "           ([name]\n"
                    + "           ,[image]\n"
                    + "           ,[price]\n"
                    + "           ,[title]\n"
    ...
    public void deleteProduct(int id) {
        try {
            String sql = "DELETE FROM [Product_HE161770]\n"
                    + "WHERE id = ? ";
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, id);
            stm.executeUpdate();
    ...
    public void updateProduct(String name, String img, String price, String title, String description, String cid, String id) {
        try {
            String sql = "UPDATE [Product_HE161770]\n"
                    + "   SET [name] = ?\n"
                    + "      ,[image] = ?\n"
                    + "      ,[price] = ?\n"
                    + "      ,[title] = ?\n"
    ...
    public List<Product> getAllProductsLast() {
        List<Product> list = new ArrayList<>();
        try {
            String sql = "SELECT TOP 4 * FROM Product_HE161770 ORDER BY ID DESC";
            PreparedStatement stm = connection.prepareStatement(sql);
            ResultSet rs = stm.executeQuery();
            while (rs.next()) {
                Product product = new Product();
